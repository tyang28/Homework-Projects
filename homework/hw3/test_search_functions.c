...
void test_find_right(){
  char grid[10][10];
  int n = populate_grid(grid, "grid.txt");
  assert(find_right(grid, n, "key", stdout) == 1); 
...
void test_find_left(){
  char grid[10][10];
  int n = populate_grid(grid, "grid.txt");
  assert(find_right(grid, n, "tip", stdout) == 1);  
}
...
void test_find_down(){
  ...
  char grid[10][10];
  int n = populate_grid(grid, "grid.txt");
  assert(find_down(grid, n, "pop", stdout) == 1);
...
void test_find_up(){
 char grid[10][10];
 int n = populate_grid(grid, "grid.txt");
 assert(find_up(grid, n, "pop", stdout) == 1);
...
void test_find_all(){
  char grid[10][10];
  int n = populate_grid(grid, "grid.txt");
  assert(find_all(grid, n, "tip", stdout) == 1);
